...
class ConvCapsule(nn.Module):
    ...
    def forward(self,in_capsules,ITER=3):
        ...
        predictions = predictions.view(batch_size, self.in_caps, self.out_caps, self.out_dim, H, W)
        out_capsules, cij, x= self.dynamic_routing(predictions,ITER)
        return out_capsules, cij, x
    ...
    def dynamic_routing(self,predictions,ITER=3):
        ...
        for it in range(ITER):
            ...
            if it < ITER - 1: 
               ...
               b_ij = b_ij + delta
        return v_j.squeeze(dim=1), c_ij, self.unif_act_wt_entropy(c_ij)
...
class ResnetCnnsovnetDynamicRouting(nn.Module):
    ...
    def forward(self,x,target=None):
        ...
        conv_caps1, cij_1, _ = self.conv_caps1(primary_caps)
        #print(f"conv_caps1.shape : {conv_caps1.shape}")
        conv_caps2, cij_2, _ = self.conv_caps2(conv_caps1)
        #print(f"conv_caps2.shape : {conv_caps2.shape}")
        conv_caps3, cij_3, _ = self.conv_caps3(conv_caps2)
        conv_caps4, cij_4, _ = self.conv_caps4(conv_caps3)
        class_caps, cij_5, x = self.class_caps(conv_caps4)
        #print(f"class_caps.shape : {class_caps.shape}")
        #print(torch.norm(conv_caps4,dim=2).cpu().detach().squeeze().numpy())
        #print(x.cpu())
        #print(cij_5.cpu().detach().squeeze().numpy())
        #assert(False)
...
def capsule_im():
    ...
    testset = torchvision.datasets.ImageFolder(root='../../data/test_2/', transform=transform_test)
    testloader = torch.utils.data.DataLoader(testset, batch_size=64, shuffle=True, num_workers=2)
    model = nn.DataParallel(ResnetCnnsovnetDynamicRouting()).to(DEVICE)
    model.load_state_dict(torch.load('checkpoints/epoch_49_trial_2.pth')['model'])
    ...
    with torch.no_grad():
         for batch_idx, (data, target) in enumerate(testloader):
             ...
             target = one_hot(target)
             output, cij_5 = model(data,target)
             if batch_idx == 0:
    ...
    map_caps = map_caps.sum(dim=1).squeeze().detach().cpu().numpy()
    #print(map_caps)
    ...
    ax[1].imshow(cv2.resize(map_caps,(data.shape[2],data.shape[3])),alpha=1,cmap='jet')
    plt.show()'''
