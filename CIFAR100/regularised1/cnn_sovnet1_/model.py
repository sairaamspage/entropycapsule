...
class PrimaryCapsules(nn.Module):
    def __init__(self,in_channels,num_capsules,out_dim,H=8,W=8):
        ...
        self.preds = nn.Sequential(
                                   nn.Conv2d(in_channels,num_capsules*out_dim,kernel_size=1,stride=2),
...
class ResnetCnnsovnetDynamicRouting(nn.Module):
    def __init__(self,analysis=False):
        ...
        self.primary_caps = PrimaryCapsules(128,32,16,8,8)#for cifar10, H,W = 16, 16. For MNIST etc. H,W = 14,14.
        self.conv_caps1 = ConvCapsule(in_caps=32,in_dim=16,out_caps=32,out_dim=16,kernel_size=3,stride=1,padding=0,analysis=analysis)
        ...
        self.class_caps = ConvCapsule(in_caps=32,in_dim=16,out_caps=100,out_dim=16,kernel_size=4,stride=1,padding=0,analysis=analysis)
    ...
    def forward(self,x):
        ...
        if self.analysis == False:
           ...
           #entropy += temp.mean()
           #print(entropy.size())
           class_caps, temp = self.class_caps(conv_caps2)
           entropy += temp.mean()
           #print(entropy.size())
        else:
